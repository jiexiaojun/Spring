package com.allen.jdbc.framework;
 ...
 * @author Tom
 */
@SuppressWarnings({"deprecation", "unchecked", "rawtypes", "hiding", "unused"})
public abstract class BaseDaoSupport<T extends Serializable, PK extends Serializable> {
	...
	protected List<T> getAll() throws Exception {
		...
		return this.jdbcTemplateReadOnly().query(sql, this.op.rowMapper,
	 ...
	 * @return 符合条件的所有对象
	 */
	protected List<Map<String, Object>> findBySql(String sql, Map<String, ?> pamam)
			throws Exception {
	 ...
	 */
	protected Map<String, Object> findUniqueBySql(String sql, Map<String, ?> pamam)
			throws Exception {
	 ...
	 * @return 符合条件的唯一对象，没符合条件的记录返回null.
	 */
	protected Map<String, Object> findUniqueBySql(String sql, List<Object> listParam)
			throws Exception {
	...
	protected Page<T> find(QueryRule queryRule, final int pageNo, final int pageSize)
		...
		String orderSql = bulider.getOrderSql();
		orderSql = (StringUtils.isEmpty(orderSql) ? " " : (" order by " + orderSql));
		String sql = "select " + op.allColumn + " from " + getTableName() + whereSql + orderSql
	 ...
	 * @return
	 */
	protected Page<Map<String, Object>> findBySqlToPage(String sql, Map<String, ?> param,
		...
		List<Map<String, Object>> list =
				(List<Map<String, Object>>) this.jdbcTemplateReadOnly().queryForList(sql, param);
		log.debug(sql);
	 ...
	 * @return
	 */
	protected Page<Map<String, Object>> findBySqlToPage(String sql, Object[] param,
			final int pageNo, final int pageSize) throws Exception {
		String countSql = "select count(1) from (" + sql + ") a";
		long count = this.jdbcTemplateReadOnly().queryForLong(countSql, param);
		if (count == 0) {
	 ...
	 */
	protected void mergeList(List<T> pojoList, List<T> poList, String idName, boolean isCopyNull)
			throws Exception {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				for (int j = 1; j <= colCount; j++) {
					...
					try {
						...
						log.warn("BeanUtils.copyProperty error, field name: " + f.getName()
								+ ", error: " + e);
					}
	 ...
	 */
	private <T> T queryForObject(String sql, RowMapper<T> mapper, Object... args) {
	...
	protected void setBlobColumn(PreparedStatement stmt, int parameterIndex, byte[] value)
			throws SQLException {
		if (value == null) {
	...
	protected void setClobColumn(PreparedStatement stmt, int parameterIndex, String value)
			throws SQLException {
		if (value == null) {
			stmt.setNull(parameterIndex, Types.CLOB);
		}
		else {
			stmt.setAsciiStream(parameterIndex, new ByteArrayInputStream(value.getBytes()),
					value.length());
		}
	}
	 ...
	 * @return
	 */
	private <T> Page simplePageQuery(String sql, RowMapper<T> rowMapper, Map<String, ?> args,
	 ...
	 * @return
	 */
	private Page simplePageQueryNotT(String sql, RowMapper rm, Map<String, ?> args, long pageNo,
			long pageSize) {
		// 首先查询总数
		String countSql = "select count(*) " + removeSelect(removeOrders(sql));
		long count = this.jdbcTemplateReadOnly().queryForLong(countSql, args);
...
//			return clazz.cast(key);
	 ...
	 * @return
	 */
	private int doUpdate(String tableName, String pkName, Object pkValue,
			Map<String, Object> params) {
	...
	private Map<String, Object> convertMap(Object obj) {
		...
		for (int i = 0, len = getters.size(); i < len; i++) {
			...
			try {
				...
				log.error(String.format("convertMap error object:%s  field: %s", obj.toString(),
						name));
			}
		}
