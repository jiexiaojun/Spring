...
public class SqlBulider {
	...
	public String buliderForInsert(String tableName, Map<String, Object> params) {
		if (null == tableName || tableName.trim().length() == 0 || params == null
				|| params.isEmpty()) {
			return "";
		}
	 ...
	 * @throws Exception
	 */
	public String buliderForInsertAll(String tableName, Map<String, PropertyMapping> fm,
			List<?> params, Object[] values) throws Exception {
		if (null == tableName || tableName.trim().length() == 0 || params == null
				|| params.isEmpty()) {
			return "";
		}
		...
		for (int j = 0; j < params.size(); j++) {
			...
			valstr.append("(");
			int k = 0;
			for (PropertyMapping p : fm.values()) {
		...
		sb.append("(" + allColumn.toString().replaceAll("\\[|\\]", "") + ") values ")
				.append(valstr.toString());
		return sb.toString();
	}
