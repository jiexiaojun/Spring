...
function initialPage() {
	...
	// 状态下拉选择
	$('.statusSelect').selectInitEx('请选择任务状态', false);
}
	...
	$('#dataGrid').bootstrapTableEx({
		...
		columns : [ {
			...
			formatter : function(value, row, index) {
                ...
                if (hasPermission('quartz:job:disable') && row.status=='1') {
                    _html += '<a href="javascript:;" onclick="vm.disable(\''+row.jobId+'\')" title="暂停任务"><i class="fa fa-stop"></i></a>';
                }
                if (hasPermission('quartz:job:enable') && row.status=='0') {
                    _html += '<a href="javascript:;" onclick="vm.enable(\''+row.jobId+'\')" title="恢复任务"><i class="fa fa-play"></i></a>';
                }
