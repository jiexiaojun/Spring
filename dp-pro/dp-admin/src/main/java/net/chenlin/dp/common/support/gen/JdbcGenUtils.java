package net.chenlin.dp.common.support.gen;
...
import net.chenlin.dp.common.utils.VelocityUtils;
...
public class JdbcGenUtils {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String jdbcUrl = "jdbc:mysql://127.0.0.1:3306/dp-pro?useUnicode=true&characterEncoding=utf-8";
        ...
        generatorCode(jdbcDriver, jdbcUrl, jdbcUsername, jdbcPassword, tablePrefix, javaProject, javaModule, webModule);
    ...
    public static void generatorCode(String jdbcDriver,
        ...
        while(tableIterator.hasNext()) {
            ...
            while(columnIterator.hasNext()){
                ...
                // 属性类型
                columnEntity.setFieldType(PropertiesUtils.getInstance("generator/generator").get(columnEntity.getDataType()));
        ...
        for (TableEntity tableEntity : tables) {
            ...
            map.put("package", PropertiesUtils.getInstance("generator/generator").get("package"));
            map.put("module", javaModule);
            map.put("author", PropertiesUtils.getInstance("generator/generator").get("author"));
            map.put("email", PropertiesUtils.getInstance("generator/generator").get("email"));
            VelocityContext context = new VelocityContext(map);

            System.out.println("============ start table: " + tableEntity.getTableName() + " ================");

            for (String template : GenUtils.getTemplates()) {
                String filePath = getFileName(template, javaProject, javaModule, webModule, tableEntity.getClassName(), rootPath);
    ...
    public static String getFileName(String template, String javaProject, String javaModule, String webModule, String className, String rootPath) {
        String packagePath = rootPath + getProjectPath(javaProject) + "/src/main/java/" + PropertiesUtils.getInstance("generator/generator").get("package").replace(".","/") + "/" + javaModule + "/";
        String resourcePath = rootPath + getProjectPath(javaProject) + "/src/main/resources/" + PropertiesUtils.getInstance("generator/generator").get("package").replace(".","/") + "/" + javaModule + "/";
        String webPath = rootPath + getProjectPath(javaProject) + "/src/main/webapp/";
        ...
        if (template.contains(GenConstant.SQL_MENU)) {
            return rootPath + getProjectPath(javaProject) + "/src/main/resources/generator/sqls/" + className.toLowerCase() + "_menu.sql";
        }
