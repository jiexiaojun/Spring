package com.allen.framework.servlet;
...
import com.allen.framework.context.GPApplicationContext;

@SuppressWarnings("serial")
public class GPDispatcherServlet extends HttpServlet {

	private static final String LOCATION = "contextConfigLocation";// 对应web.xml中的init-param的param-name
				...
				while (null != (line = ra.readLine())) {
					...
					while (m.find()) {
						for (int i = 1; i <= m.groupCount(); i++) {
							...
							line = line.replaceAll("@\\{" + paramName + "\\}",
									paramValue.toString());
						}
					}
